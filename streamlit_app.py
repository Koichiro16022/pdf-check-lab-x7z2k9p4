...
from modules.sheet_comparator import SheetComparator
...
# --- 👁️ 突き合わせプレビュー（左右2画面・ピンポイント詳細のみ） ---
    ...
    with col_scan2:
        if st.button("🔍 スキャン開始", type="primary", use_container_width=True):
		    ...
		    # --- 🔍 スキャン実行（全域カバー版へ修正） ---
                ...
                setting_diffs = []
                try:
                    sheet_comp = SheetComparator()
                    setting_diffs = sheet_comp.compare_books(
                        loader_m.wb, loader_c.wb,
                        loader_m.sheet_names[0], loader_c.sheet_names[0]
                    )
                except Exception:
                    pass
                ...
                current_result = {
                    ...
                    'total_cells': int(page_size) * actual_max_col,
                    'matched': int(page_size) * actual_max_col - len(cell_diffs) - len(hidden_diffs),
    ...
    if st.session_state.scan_results:
        ...
        if all_diffs:
            ...
            for diff_idx, diff in enumerate(current_diff['differences']):
                with st.expander(f"📌 {diff['type']}: {diff['detail']}", expanded=True):
                    col_d1, col_d2 = st.columns(2)
                    with col_d1:
                        st.text("📄 原本:")
                        st.text_area("", value=str(diff.get('master', '')), height=120, disabled=True, label_visibility="collapsed", key=f"master_{st.session_state.current_diff_index}_{diff_idx}")
                    with col_d2:
                        st.text("📝 比較:")
                        st.text_area("", value=str(diff.get('check', '')), height=120, disabled=True, label_visibility="collapsed", key=f"check_{st.session_state.current_diff_index}_{diff_idx}")
        ...
        # シート・ブック設定差異
        setting_diffs = results.get('setting_diffs', [])
        st.markdown("---")
        st.subheader(f"⚙️ シート・ブック設定差異 ({len(setting_diffs)}件)")
        if setting_diffs:
            import pandas as pd
                ...
                with st.spinner("Excel生成中（2ファイル）..."):
                    try:
                        ...
                        for result in st.session_state.accumulated_results:
                            ...
                            # 設定差異は最後のスキャン結果を使用（重複防止）
                        all_setting_diffs = st.session_state.scan_results.get('setting_diffs', [])
